...
//        explicit BasicIterator(const Node node) : node_(&node) {}
...
public:
    ...
    SingleLinkedList(const SingleLinkedList& other) {
        ...
        Node* node_other = other.head_->next_node;
        
        while(node_other) {
            //copy node
            node_tmp = new Node(node_other->value);
            //advance to next nodes
            node_tmp = node_tmp->next_node;
            node_other = node_other->next_node;
        }
...
//======= Insert & delete ========//
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        if(!pos.node_->next_node) {
            return end();
        } //get node after next (next node will be deleted)
